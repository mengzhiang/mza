(function() {
	...
	function timeit() {}
	timeit.realtime = true;
	timeit.logs = [];
	timeit.start = function() {
		timeit.logs.push({name: '', time: new Date().getTime()});
		if (timeit.realtime) console.log('计时开始');
	}
	timeit.log = function(name) {
		var time = new Date().getTime();
		timeit.logs.push({name: name, time: time});
		var last = timeit.logs.length - 2;
		if (last < 0) last = 0;
		if (timeit.realtime) console.log((time - timeit.logs[last].time) + ': ' + name);
	}
	timeit.test = function() {
		if (!timeit.realtime) {
			var str = '';
			for (var i = 1; i < timeit.logs.length; i++) {
				str += (timeit.logs[i].time - timeit.logs[i - 1].time) + ': ' + timeit.logs[i].name + '\n';
			}
			str += (timeit.logs[timeit.logs.length - 1].time - timeit.logs[0].time) + '总共用时';
			console.log(str);
		}
		timeit.logs = [];
	}
...
(function()
{
		...
		*/
		...
		clearEvents : function() {
			for (var eventInfo, i = 0; eventInfo = allEvents[i]; i++) {
				XN.event.delEvent.apply(XN.event, eventInfo);
			}
        ...
        {
			if (XN.event.ignoreEvent) return;
            ...
            {
				if (XN.event.logEvents) allEvents.push([v, name, func, cap]);
	...
	if (window.attachEvent && !browser.Opera)
	{
		...
		{
            element = $( element );
			if (!element._eventListeners[name]) return;
			if (name == 'keypress') name = 'keydown';
			if (name == 'input' ) name = 'propertychange';
...
(function()
{
	...
	XN.dom = {
		...
		nativeReady : function(f, async)
		{
            if ( isUndefined(async) ) async = false;
			if ( domLoaded )
            {
                return async ? setTimeout(f, 0) : f();
            }
            else
            {
...
(function()
{
    ...
    XN.loadFile = function( file , callBack , disCache ){
		return XN.dom.nativeReady(function(){
			return loadFile( file , callBack , disCache );	
		});        
    };

	XN.unloadFile = function(node) {
		if (node.parentNode) {
	...
	XN.clearFiles = function() {
		for (var i in files) if (files.hasOwnProperty(i)) {
			if (files[i] && files[i].element) XN.unloadFile(files[i].element);
		}
	}
...
(function()
{
	...
	XN.element = {
		...
		delegate: function(element, selector, type, callback) {
			$(element).addEvent(type, function(e) {
...
XN.util.clearAllTimer = function() {
	...
	for (var i = 0; i < this.__intervals.length; i++) clearTimeout(this.__intervals[i]);
	this.__timeouts = [];
	this.__intervals = [];
}
...
(function(){
    ...
    XN.DO.alert = function( message , title , type , X , Y , w , h , callBack ){
        ...
        else if ( isString( message ) || arguments.length > 1 ){
            var ars = arguments;
...
(function()
{
    ...
    {
        if ( !window.ad_js_version ) return;
...
{	
    ...
    {
        var el = this.element;
        if (el.value == this._defaultValue) return '';
        return el.value;
    },
...
XN.dom.ready(function()
{
	if ( $('appDropMenu') ) {
		...
		var menu = new XN.ui.menu({
			...
			offsetY: XN.browser.IE6?0:(($('showAppMenu').realTop()+35)-($('appDropMenu').realTop()+17)),
			fireOn:'mouseover',
			addIframe : true
		});
...
XN.dom.ready(function(){
	...
	var menu = new XN.ui.menu({
		...
		offsetY: XN.browser.IE6?0:(($('showProfileMenu').realTop()+35)-($('profileDropMenu').realTop()+17)),
        fireOn:'mouseover',
        addIframe : true
    });
	menu.onShow = function(){
...
XN.dom.ready(function(){
	...
	var menu = new XN.ui.menu({
		...
		offsetY: XN.browser.IE6?0:(($('showFriendMenu').realTop()+35)-($('friendDropMenu').realTop()+17)),
        fireOn:'mouseover',
        addIframe : true
    });
	menu.onShow = function(){
...
var object = new (function(globalHost) {
	...
	// 仅保存模块名，import时再生成对象，避免import时引入不需要的模块
	this.add = function(name, context) {

		var names = name.split('.');
		for (var i = 0; i < names.length; i++) {
			var name = names.slice(0, i + 1).join('.');
			if (!_lib[name]) _lib[name] = {};
		}

		var module = _lib[name];
		context.call(module, module);

		// 为module中的成员生成名字
		for (var i in module) {
			module[i].__name__ = i;
			module[i].__module__ = name;
		}
	};

	function _loadFile(src, callback) {
		var ua = _lib['ua'];
        ...
        if (ua.ie) {
            ...
            ele.onload = ele.onerror = function() {
				callback();
            };
        }
	...
	/**
	 * @param names 数组，是一个module的name的每个部分
	 * @param as 别名
	 * @param root 可选的root，最终use进来的module会被附加到这个root上
	 * @param after 在动态加载中，加载完毕后会调用的函数
	 * @returns 一个包含了引入的module的对象，注意，不是直接返回了module，而是包含了module的root
	 */
	var _use = function(names, as, root, after) {
		// root是一个容器，里面放着本次use进来的module，当然里面有可能已经有一些module了
		if (!root) root = {};
		...
		if (as) {
			...
			object.extend(module, _lib[name]);
			root[as] = module;
		} else {
			for (var i = 0, module = root; i < names.length; i++) {
				var name = names.slice(0, i + 1).join('.');
				if (!module[names[i]]) module[names[i]] = new Module(name);
				if (_lib[name]) { // lib中有
					if (_lib[name].__file__) { // 进行动态加载
						loadFile(_lib[name].__file__, function() {
							after();
						});
					} else {
						object.extend(module[names[i]], _lib[name]);
	...
	/**
	 * 这个方法有两种使用情景，一种传入第二个参数context，context的参数会包含use进来的module，如果没有第二个参数，则会返回需要引入的module
	 * @param uses 如果有context参数，则为用逗号分隔开的模块名称列表，如果没有context参数，则为一个唯一的模块名称，而非列表，模块可以使用 : 表示其别名
	 * @param context 如果有这个参数，这个function会在所有module准备完毕后调用，并将module通过参数传入context，第一个参数为所有module的引用，后面的参数为每个module的不重复引用，顺序排列
	 * @return 在没有context参数的情况下，返回module
	 */
	this.use = function(uses, context) {

		// 如果没有传入context，则直接返回module
		if (!context) {
			var name = uses.split(':');
			// 别名
			var as = name[1];
			...
			// _use() 的返回值是一个容器，其中的唯一的成员才是需要引入的module，这个唯一成员的名字即为 names[0]
			return _use(names, as)[names[0]];

		} else {
			var uses = uses.split(/\s*,\s*/ig);
			var args = [];
			var ns = {};
			// 第一个参数是ns，ns是所有use的module
			args.push(ns);

			for (var i = 0; i < uses.length; i++) {
				var name = uses[i].split(':');
				// 别名
				var as = name[1];
				name = name[0];
				var names = name.split('.');
				...
				// 这里的as为需要被放到ns上的名称，并非真正意义上的as
				if (!as) as = names[0];
				if (args.indexOf(ns[as]) == -1) args.push(ns[as]);
			}

			context.apply(ns, args);
		}
	...
	// 类
	this.$class = object.Class = function() {
	...
	this.staticmethod = function(func) {
		func.__self__ = null;
		return func;
	}

	// 声明类方法，在new Class(callback) 的callback中调用
	this.classmethod = function(func) {
	...
	this.Options = new object.Class({
		...
		setOption: function(self, name, type, value) {
			if (value !== undefined) {
				...
				var value = self._provider.makeOption(name, type);
	...
	if (!Array.prototype.every) {
		Array.prototype.every = function(object, fn, bind){
	...
	if (!Array.prototype.filter) {
		Array.prototype.filter = function(object, fn, bind){
			var results = {};
	...
	// dom ready
	this.ready = function() {
		return _lib['dom'].ready.apply(this, arguments);
	}

	//this.$ = function() {
		//return _lib['dom'].$.apply(this, arguments);
	//}
...
this.$ = function(id) {
	...
	else if (typeof id === 'string' || typeof id === 'number') element = document.getElementById(id);
	else element = id;
	if (element) Element.wrap(element);
	return element || null;
}
 ...
 */
this.$$ = function() {
	var eles = Sizzle.apply(this, arguments);
...
var eval_inner_JS = this.eval_inner_JS = function(ele) {
	var js = ele.getElementsByTagName('script');
...
var Element = this.Element = new Class(function() {
	...
	t.innerHTML = '<TEST_TAG></TEST_TAG>';
	// IE 下无法获取到自定义的Element，其他浏览器会得到HTMLUnknownElement
	this.needGetDom = t.firstChild === null;
	...
	this.getElements = function(self, selector, cls) {
		...
		return new ns.Elements(eles, cls);
	}
	...
	this.wrap = classmethod(function(cls, ele) {
		...
		for (var i in cls.prototype) if (cls.prototype[i].im_func === undefined) ele[i] = cls.prototype[i];
		cls.__init__(ele);
...
object.add('net', function() {

var Ajax = this.Ajax = new Class(function() {
 ...
 * @class
 */
var TabControl = this.TabControl = new Class(dom.Element, function() {
		...
		self.tabs.forEach(function(ele, i) {
			...
			ele.addEvent('click', function() {
				self.tabs.forEach(function(tab, i) {
					dom.$(tab).classList.remove('selected');
				});
