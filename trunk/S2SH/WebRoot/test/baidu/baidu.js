...
baidu.date.parse = function(C) {
	...
	if ("string" == typeof C) {
		if (A.test(C) || isNaN(Date.parse(C))) {
			var E = C.split(/ |T/), B = E.length > 1 ? E[1].split(/[^\d]/) : [
					0, 0, 0], D = E[0].split(/[^\d]/);
			return new Date(D[0] - 0, D[1] - 1, D[2] - 0, B[0] - 0, B[1] - 0,
					B[2] - 0)
		} else {
			return new Date(C)
		}
...
baidu.dom._styleFixer.textOverflow = (function() {
	...
	function D(M, H, Q) {
		var J = baidu.browser.ie
				? M.currentStyle || M.style
		...
		for (var K = M.firstChild, N = E[256], G = E[257], F = E[258], S = [], Q = Q
				? F
				: 0; K; K = K.nextSibling) {
			if (H < Q) {
				M.removeChild(K)
	...
	return {
		get : function(F, G) {
			var E = baidu.browser;
			return (E.opera ? G.OTextOverflow : E.firefox
					? F._baiduOverflow
					: G.textOverflow)
					|| "clip"
		},
		set : function(F, H) {
			...
			I = F.style;
			E.opera ? (I.OTextOverflow = H) : E.firefox
					? (F._baiduOverflow = H)
					: (I.textOverflow = H)
		}
	}
...
baidu.dom.removeClass = function(B, C) {
	...
	B.className = A(B.className.split(/\s+/).join("  ").replace(
			new RegExp("(^| )(" + A(C).split(/\s+/).join("|") + ")( |\x24)",
					"g"), " ").replace(/\s+/g, " "));
	return B
};
...
baidu.dom.getPosition = function(B) {
	var H = baidu.dom.getDocument(B), E = baidu.browser;
	B = baidu.dom.g(B);
	var C = E.isGecko > 0 && H.getBoxObjectFor
			&& baidu.dom.getStyle(B, "position") == "absolute"
			&& (B.style.top === "" || B.style.left === "");
	var G = {
		left : 0,
		top : 0
	};
	var F = (E.ie && !E.isStrict) ? H.body : H.documentElement;
	if (B == F) {
		return G
	}
	var I = null;
	...
	if (B.getBoundingClientRect) {
		...
		G.left = Math.floor(D.left)
				+ Math.max(H.documentElement.scrollLeft, H.body.scrollLeft);
		G.top = Math.floor(D.top)
				+ Math.max(H.documentElement.scrollTop, H.body.scrollTop);
		G.left -= H.documentElement.clientLeft;
		G.top -= H.documentElement.clientTop;
		if (E.ie && !E.isStrict) {
			G.left -= 2;
			G.top -= 2
		}
	} else {
		if (H.getBoxObjectFor && !C) {
			D = H.getBoxObjectFor(B);
			var A = H.getBoxObjectFor(F);
			G.left = D.screenX - A.screenX;
			G.top = D.screenY - A.screenY
		} else {
			I = B;
			do {
				G.left += I.offsetLeft;
				G.top += I.offsetTop;
				if (E.isWebkit > 0
						&& baidu.dom.getStyle(I, "position") == "fixed") {
					G.left += H.body.scrollLeft;
					G.top += H.body.scrollTop;
					break
				}
				I = I.offsetParent
			} while (I && I != B);
			if (E.opera > 0
					|| (E.isWebkit > 0 && baidu.dom.getStyle(B, "position") == "absolute")) {
				G.top -= H.body.offsetTop
			}
			I = B.offsetParent;
			while (I && I != H.body) {
				G.left -= I.scrollLeft;
				if (!b.opera || I.tagName != "TR") {
					G.top -= I.scrollTop
				}
				I = I.offsetParent
			}
...
baidu.dom.intersect = function(H, G) {
	...
	G = F(G);
	var D = E(H), B = E(G);
	return A(D.left, B.left) <= C(D.left + H.offsetWidth, B.left
					+ G.offsetWidth)
			&& A(D.top, B.top) <= C(D.top + H.offsetHeight, B.top
							+ G.offsetHeight)
};
...
baidu.dom.ready = function() {
	...
	function B() {
		...
		G.attachEvent ? G.attachEvent("onload", A) : G.addEventListener("load",
				A, false)
	}
	return function(F) {
		B();
...
baidu.ajax.request = function(D, O) {
	...
	var I = O.data || "", G = !(O.async === false), H = O.username || "", M = O.password
			|| "", A = (O.method || "GET").toUpperCase(), F = O.headers || {}, B = {}, L, N;
	for (L in O) {
		B[L] = O[L]
	}
...
baidu.ajax.form = function(A, C) {
	C = C || {};
	var F = A.elements, M = F.length, B = A.getAttribute("method"), E = A
			.getAttribute("action"), S = C.replacer || function(U, T) {
		return U
...
baidu.swf.version = (function() {
	...
	if (F.plugins && F.mimeTypes.length) {
		...
		if (C && C.description) {
			return C.description.replace(/([a-zA-Z]|\s)+/, "").replace(
					/(\s)+r/, ".")
					+ ".0"
		}
	} else {
...
baidu.swf.createHTML = function(P) {
	...
	var K = P.vars, J = ["classid", "codebase", "id", "width", "height",
			"align"];
...
baidu.string.format = function(C, A) {
	...
	if (B.length) {
		B = B.length == 1 ? (A !== null
				&& (/\[object Array\]|\[object Object\]/.test(D.call(A)))
				? A
				: B) : B;
...
baidu.page.getHeight = function() {
	var D = document, A = D.body, C = D.documentElement, B = D.compatMode == "BackCompat"
			? A
			: D.documentElement;
...
baidu.page.getViewWidth = function() {
	var B = document, A = B.compatMode == "BackCompat"
			? B.body
			: B.documentElement;
...
(function() {
	...
	function A(Q) {
		var M = H.range, P = baidu.page.getMousePosition(), N = E + P.x - D, O = K
				+ P.y - C;
...
(function() {
	...
	var B = function(V, Q, Y, Z) {
		...
		if (W.length > 1 && J.exec(V)) {
			...
			if (!Z && W.length > 1 && Q.nodeType === 9 && !T
					&& E.match.ID.test(W[0])
					&& !E.match.ID.test(W[W.length - 1])) {
			...
			if (Q) {
				d = Z ? {
					expr : W.pop(),
					set : A(Z)
				} : B.find(W.pop(), W.length === 1
								&& (W[0] === "~" || W[0] === "+")
								&& Q.parentNode ? Q.parentNode : Q, T);
	...
	var E = B.selectors = {
		...
		relative : {
			"+" : function(W, R) {
				var T = typeof R === "string", V = T && !/\W/.test(R), X = T
						&& !V;
				if (V) {
					R = R.toLowerCase()
				}
				for (var S = 0, Q = W.length, U; S < Q; S++) {
					if ((U = W[S])) {
						...
						W[S] = X || U && U.nodeName.toLowerCase() === R
								? U || false
								: U === R
					}
				}
		...
		preFilter : {
			...
			CHILD : function(Q) {
				if (Q[1] === "nth") {
					var R = /(-?)(\d*)n((?:\+|-)?\d*)/.exec(Q[2] === "even"
							&& "2n" || Q[2] === "odd" && "2n+1"
							|| !/\D/.test(Q[2]) && "0n+" + Q[2] || Q[2]);
					Q[2] = (R[1] + (R[2] || 1)) - 0;
					Q[3] = R[3] - 0
				}
				Q[0] = I++;
		...
		filters : {
			...
			button : function(Q) {
				return "button" === Q.type
						|| Q.nodeName.toLowerCase() === "button"
			},
			input : function(Q) {
		...
		filter : {
			PSEUDO : function(S, X, W, Y) {
				...
				if (R) {
					...
					if (Q === "contains") {
						return (S.textContent || S.innerText || B.getText([S]) || "")
								.indexOf(X[3]) >= 0
					} else {
						if (Q === "not") {
							var T = X[3];
			...
			TAG : function(R, Q) {
				return (Q === "*" && R.nodeType === 1)
						|| R.nodeName.toLowerCase() === Q
			},
			CLASS : function(R, Q) {
			...
			ATTR : function(V, T) {
				var S = T[1], Q = E.attrHandle[S]
						? E.attrHandle[S](V)
						: V[S] != null ? V[S] : V.getAttribute(S), W = Q + "", U = T[2], R = T[4];
				return Q == null
						? U === "!="
						: U === "="
								? W === R
	...
	for (var L in E.match) {
		E.match[L] = new RegExp(E.match[L].source
				+ (/(?![^\[]*\])(?![^\(]*\))/.source));
		E.leftMatch[L] = new RegExp(/(^(?:.|\r|\n)*?)/.source
				+ E.match[L].source.replace(/\\(\d+)/g, F))
	}
	var A = function(R, Q) {
	...
	(function() {
		...
		if (Q.firstChild && typeof Q.firstChild.getAttribute !== "undefined"
	...
	(function() {
		...
		if (!Q.getElementsByClassName
				|| Q.getElementsByClassName("e").length === 0) {
			return
		}
...
baidu.page.createStyleSheet = function(A) {
	...
	if (baidu.browser.ie) {
		...
		F.url
				&& (C = "<link type='text/css' rel='stylesheet' href='" + F.url
						+ "'/>");
		baidu.dom.insertHTML(D.getElementsByTagName("HEAD")[0], "beforeEnd", C);
		var B = D.styleSheets[D.styleSheets.length - 1], E = B.rules
				|| B.cssRules;
		return {
			self : B,
			rules : B.rules || B.cssRules,
			addRule : function(G, I, H) {
				if (B.addRule) {
